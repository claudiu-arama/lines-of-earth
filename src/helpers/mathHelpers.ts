...
const getSquaredSegmentDistance = (
  ...
  // set a "vector"
  const deltaX = segmentEnd[0] - startX;
  const deltaY = segmentEnd[1] - startY;

  // if sengment is not a point
  if (deltaX !== 0 || deltaY !== 0) {
    const prejection =
      ((point[0] - startX) * deltaX + (point[1] - startY) * deltaY) /
...
export const simplifyPath = (
  ...
  const squaredTolerance =
    distanceThreshold !== undefined ? distanceThreshold * distanceThreshold : 1;

  const lastIndex = allPoints.length - 1;

  // keep 1st point
  const simplifiedResults = [allPoints[0]];
